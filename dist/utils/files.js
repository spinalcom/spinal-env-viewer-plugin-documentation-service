...
function convertFileToSpinalFile(files) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        let filePath;
        if (element.buffer)
            filePath = new spinal_core_connectorjs_type_1.Path(element.buffer, src_1.FileExplorer.getMimeType(element.name));
        else
            filePath = new spinal_core_connectorjs_type_1.Path(element, src_1.FileExplorer.getMimeType(element.name));
...
function addChildrenToNode(parentNode, childNode, relationName, contextNode) {
    return parentNode.addChildInContext(childNode, relationName, spinal_env_viewer_graph_service_1.SPINAL_RELATION_PTR_LST_TYPE, contextNode).then(async (result) => {
        if (parentNode.getType().get() === src_1.DIRECTORY_NODE_TYPE) {
