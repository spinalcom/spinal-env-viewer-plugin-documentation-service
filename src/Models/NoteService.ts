...
class NoteService {
   ...
   */
  public async addNote(node: SpinalNode<any>, userInfo: { username: string; userId: number }, note: string, type?: string, file?: SpinalFile<spinal.Model>, noteContextId?: string, noteGroupId?: string, viewPoint?: ViewStateInterface): Promise<SpinalNode<any>> {
    if (!(node instanceof SpinalNode)) throw "node must be a SpinalNode";
    if (file && !(file instanceof SpinalFile)) throw "File must be a SpinalFile";
