...
import { SpinalGraphService, SPINAL_RELATION_PTR_LST_TYPE, SpinalNode } from "spinal-env-viewer-graph-service";
import { SpinalAttribute } from "spinal-models-documentation";
import type { ICategory } from "../interfaces";
import { ATTRIBUTE_TYPE, BUILDINGINFORMATION, BUILDINGINFORMATIONCATNAME, CATEGORY_TYPE, NODE_TO_ATTRIBUTE, NODE_TO_CATEGORY_RELATION } from "./constants";
...
class AttributeService {
	 ...
	 * @memberof AttributeService
	 */
	public async delCategoryAttribute(node: SpinalNode<any>, serverId: number): Promise<void> {
		if (!(node instanceof SpinalNode)) throw new Error("Node must be a SpinalNode.");
	 ...
	 * @memberof AttributeService
	 */
	public async deleteAttributeCategory(node: SpinalNode<any>, category: SpinalNode<any> | ICategory | string): Promise<void> {
		let _category;
		...
		if (_category instanceof SpinalNode) return node.removeChild(_category, NODE_TO_CATEGORY_RELATION, SPINAL_RELATION_PTR_LST_TYPE);
	 ...
	 * @memberof AttributeService
	 */
	public async editCategoryAttribute(node: SpinalNode<any>, serverId: number, categoryName: string): Promise<void> {
		categoryName = categoryName.toString().trim();
	 ...
	 * @memberof AttributeService
	 */
	public async getAllAttributes(node: SpinalNode<any>): Promise<SpinalAttribute[]> {
		const categories = await this.getCategory(node);
	 ...
	 * @memberof AttributeService
	 */
	public async findOneAttributeInCategory(node: SpinalNode<any>, category: string | ICategory, label: string = ""): Promise<SpinalAttribute | -1> {
		label = label.toString().trim();
		if (!(node instanceof SpinalNode)) throw new Error("node must be a spinalNode instance");
	 ...
	 * @memberof AttributeService
	 */
	public async getAttributesByCategory(node: SpinalNode<any>, category: string | ICategory, label?: string): Promise<SpinalAttribute[]> {
		if (!(node instanceof SpinalNode)) throw new Error("node must be a spinalNode instance");
		const _category = typeof category == "string" ? await this.getCategoryByName(node, category) : category;
	 ...
	 * @memberof AttributeService
	 */
	public async updateAttribute(node: SpinalNode<any>, category: string | ICategory, label: string, newValues: { label?: string; value?: string; type?: string; unit?: string }, createIt: boolean = false): Promise<SpinalAttribute> {
	 ...
	 * @memberof AttributeService
	 */
	public async setAttributeById(node: SpinalNode<any>, serverId: number, new_label: string, new_value: string, new_type?: string, new_unit?: string): Promise<void> {
		new_label = new_label.toString().trim();
	 ...
	 * @memberof AttributeService
	 */
	public async getAttributesShared(node: SpinalNode<any>, categoryName?: string): Promise<{ parentNode: SpinalNode<any>; categories: ICategory[] }[]> {
		categoryName = categoryName.toString().trim();
	 ...
	 * @memberof AttributeService
	 */
	public async removeAttributesByLabel(category: ICategory, label: string): Promise<boolean> {
	 ...
	 * @memberof AttributeService
	 */
	public async removeAttributesById(category: ICategory, serverId: number): Promise<boolean> {
		const listAttributes = await category.element.load();
		for (let i = 0; i < listAttributes.length; i++) {
			const element = listAttributes[i];
			element.upgradeDate();
	 ...
	 * @memberof AttributeService
	 */
	public async getBuildingInformationAttributes(node: SpinalNode<any>): Promise<SpinalAttribute[]> {
	 ...
	 * @memberof AttributeService
	 */
	public async setBuildingInformationAttributes(node: SpinalNode<any> | string): Promise<SpinalAttribute[]> {
	 ...
	 * @memberof AttributeService
	 */
	public async findAttributesByLabel(node: SpinalNode<any>, label: string, category?: ICategory): Promise<SpinalAttribute> {
	 ...
	 */
	public async createOrUpdateAttrsAndCategories(node: SpinalNode<any>, categoryName: string, attrsToUp: Record<string, string>, shouldUpdateDirectModificationDate: boolean = false): Promise<void> {
	 ...
	 * @memberof AttributeService
	 */
	public async getAttributes(node: SpinalNode<any>): Promise<{ node: SpinalNode<any>; element: SpinalAttribute }[]> {
	 ...
	 * @memberof AttributeService
	 */
	public async _categoryExist(node: SpinalNode<any>, categoryName: string): Promise<SpinalNode<any>> {
		// const categories = await node.getChildren(NODE_TO_CATEGORY_RELATION);
		const categories = await this.getCategory(node);
